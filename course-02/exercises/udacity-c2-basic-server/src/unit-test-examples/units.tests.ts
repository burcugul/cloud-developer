...
describe('concat function', () => {
  ...
  it('should concat two and two', () => {
    const result = concat('2','2');
    expect(result).to.equal('22');
  });

  it('should concat -2 and 2', () => {
    const result = concat('-2','2');
    expect(result).to.equal('-22');
  });

  it('should throw an error if concat empty', () => {
    expect(()=>{ concat('','') }).to.throw('strings are empty')
  });
});
